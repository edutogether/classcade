...
const modules = import.meta.glob('../assets/classcade/nbti-results/nbti-result-*.png', { eager: true, import: 'default' }) as Record<string, string>

const byType: Record<string, string> = {}
for (const [path, url] of Object.entries(modules)) {
  const match = path.match(/nbti-result-([A-Z]{4})\.png$/)
