...
/** MBTI-style 4-letter type code, one letter per axis in NBTI_AXES order. `result.code`
 *  (e.g. "P01") is an internal lookup id for PROVISIONAL_NBTI_RESULTS, not meant for
 *  display - this derives the familiar-looking code straight from the four directions. */
const directionInitial: Record<NbtiDirection, string> = { design: 'D', response: 'R', whole: 'W', individual: 'I', criteria: 'C', empathy: 'E', completion: 'F', expansion: 'X' }
function nbtiTypeCode(directions: readonly NbtiDirection[]) { return directions.map((direction) => directionInitial[direction]).join('') }
