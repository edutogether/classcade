...
export function GameIntroScene(props: JourneySceneProps) {
  ...
  return (
    <SceneFrame scene="game" {...props}>
      <div className="journey-game-intro journey-enter"><p className="journey-kicker">RESULT-DEPENDENT CLASS QUEST</p><p className="journey-game-intro__badge"><Icon name="spark" size={16} />{getProvisionalResult(props.state.resultCode).title}에게 열린 퀘스트</p><h1>{variant.title}</h1><p>{variant.summary}</p><div className="journey-game-intro__facts"><span><Icon name="gamepad" size={23} />선택형 학급 게임</span><span><Icon name="clock" size={23} />약 2분 체험</span><span><Icon name="share" size={23} />완성 카드 저장</span></div><PrimaryButton onClick={() => props.onAction({ type: 'START_GAME' })}>이 퀘스트 시작하기</PrimaryButton><SecondaryButton onClick={() => props.onAction({ type: 'OPEN_RESULT' })}>NBTI 결과 다시 보기</SecondaryButton></div>
    </SceneFrame>
  )
}
