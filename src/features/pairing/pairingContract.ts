...
export type PairingStore = { create(record: PairingRecord): Promise<'created' | 'collision'>; consume(code: string, now: number): Promise<{ status: 'connected'; record: PairingRecord } | { status: 'invalid' | 'expired' | 'used' }>; revoke(code: string, now: number): Promise<'revoked' | 'not_active'> }
...
export function restorePairingJourney(payload: PairingPayload, audio: AudioSettings): JourneyState {
  return { version: 2, stage: 'nbti_result', questionIndex: 15, answers: { ...payload.answers }, resultCode: payload.resultCode, gameVariantId: getGameVariantForResult(payload.resultCode).id, gameStep: 0, gameChoices: {}, shakeProgress: 0, audio, updatedAt: new Date().toISOString(), completedAt: null, gameConditions: null, gameConcept: null, selectedGameId: null, gameAdjustments: {}, completion: { recommendationTags: [], recommendedVideoIds: [], shareCardFormat: null, shareCardGenerated: false, lastCompletedStep: null }, resumeStage: null }
}
