import { Timestamp, deleteDoc, doc, getDoc, onSnapshot, runTransaction, serverTimestamp, type Unsubscribe } from 'firebase/firestore'
...
export class FirestorePairingStore implements PairingStore {
  ...
  async revoke(code: string) {
    ...
    if (snapshot.exists() && (snapshot.data() as FirestorePairingDocument).creatorUid === user.uid) await deleteDoc(ref)
  }
}
