...
export function EntryVisualTuner({ screen }: Props) {
  const enabled = isTunerEnabled()
  const [tuning, setTuning] = useState<EntryTuning>(readSaved)
  const [open, setOpen] = useState(false)
  const [history, setHistory] = useState<EntryTuning[]>([])
  const [future, setFuture] = useState<EntryTuning[]>([])
  ...
  return <aside className={`entry-tuner ${open ? 'is-open' : ''}`} aria-label="Entry Visual Tuner">
    <button className="entry-tuner__toggle" type="button" aria-expanded={open} onClick={() => setOpen((value) => !value)}>Tune</button>
