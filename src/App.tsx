...
export default function App() {
  ...
  return <>
    ...
    {showJourney && (profile || pairingEntryRequested || sharedResultRequested) && (
      <ChunkErrorBoundary>
        <Suspense fallback={<div style={{ background: '#050c07', minHeight: '100svh' }} />}>
          <JourneyApp
            state={journeyState}
            notice={notice}
            onAction={handleJourneyAction}
            onTeacherOpen={(button) => { teacherTriggerRef.current = button; setTeacherOpen(true) }}
            teacherTriggerRef={teacherTriggerRef}
            profile={profile}
            journeyId={journeyId}
            pairingEntry={pairingEntryRequested && !profile}
            onPaired={restorePairedJourney}
            onStartHere={() => { setScreen('prep'); clearPairingEntryQuery() }}
            onExitPairingEntry={() => { setScreen('prep'); clearPairingEntryQuery() }}
            onNextParticipant={() => resetActiveJourney('laptop-next-participant')}
          />
        </Suspense>
      </ChunkErrorBoundary>
    )}
    {screen === 'prep' && <AdventurePrepScreen initialProfile={profile} audio={journeyState.audio} exiting={prepExiting} isOffline={!online} onComplete={handleProfileComplete} onScreenChange={setTunerScreen} onAudioChange={(audio) => applyJourneyAction({ type: 'SET_AUDIO', audio })} />}
