...
const target = path.join(root, 'src', 'config', 'entry-tuning.v1.json')
...
if (!source) throw new Error('Usage: npm run entry:tune:apply -- <exported-json-path>')
const value = JSON.parse(fs.readFileSync(path.resolve(source), 'utf8'))
if (value.version !== 1 || !validValues(value.global) || !screens.every((screen) => validValues(value.screens?.[screen]))) throw new Error('Invalid Entry tuning schema, token, or range.')
...
console.log(`Applied Entry tuning to ${path.relative(root, target)}`)
